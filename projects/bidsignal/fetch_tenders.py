import json
import os
import re
import urllib.request
from datetime import datetime, timezone
from html import escape
from pathlib import Path

ROOT = Path(__file__).resolve().parent
DATA_DIR = ROOT / "data"
DATA_DIR.mkdir(exist_ok=True)
PROFILES = json.loads((ROOT / "profiles.json").read_text(encoding="utf-8"))

TED_SEARCH_URL = "https://api.ted.europa.eu/v3/notices/search"

QUERY = {
    "query": "publication-number=*",
    "fields": [
        "publication-number",
        "notice-title",
        "buyer-name",
        "place-of-performance",
        "estimated-value",
        "currency",
        "deadline-receipt-tender-date-lot",
        "publication-date",
        "notice-type",
        "document-sent-date",
        "contract-nature",
    ],
    "page": 1,
    "limit": 100,
    "scope": "ACTIVE",
    "checkQuerySyntax": False,
}


def post_json(url, payload):
    body = json.dumps(payload).encode("utf-8")
    req = urllib.request.Request(
        url,
        data=body,
        headers={
            "Content-Type": "application/json",
            "Accept": "application/json",
            "User-Agent": "BidSignal/0.1",
        },
        method="POST",
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
        return json.loads(resp.read().decode("utf-8"))


def normalize_text(value):
    if value is None:
        return ""
    if isinstance(value, list):
        value = " ".join(str(x) for x in value)
    if isinstance(value, dict):
        value = " ".join(str(v) for v in value.values())
    return re.sub(r"\s+", " ", str(value)).strip()


def flatten_notice(notice):
    title = normalize_text(notice.get("notice-title"))
    buyer = normalize_text(notice.get("buyer-name"))
    place = normalize_text(notice.get("place-of-performance"))
    nature = normalize_text(notice.get("contract-nature"))
    text = " ".join([title, buyer, place, nature]).lower()
    return {
        "publication_number": normalize_text(notice.get("publication-number")),
        "title": title or "Untitled notice",
        "buyer": buyer,
        "place": place,
        "estimated_value": normalize_text(notice.get("estimated-value")),
        "currency": normalize_text(notice.get("currency")),
        "deadline": normalize_text(notice.get("deadline-receipt-tender-date-lot")),
        "publication_date": normalize_text(notice.get("publication-date")),
        "notice_type": normalize_text(notice.get("notice-type")),
        "contract_nature": nature,
        "search_text": text,
    }


def score_notice(notice, profile):
    text = notice["search_text"]
    score = 0
    hits = []

    for kw in profile.get("include", []):
        kw_l = kw.lower()
        if kw_l in text:
            score += 3 if kw_l in notice["title"].lower() else 1
            hits.append(kw)

    for kw in profile.get("exclude", []):
        if kw.lower() in text:
            score -= 5

    countries = [c.lower() for c in profile.get("countries", [])]
    if countries:
        if any(c in notice["place"].lower() for c in countries):
            score += 2
        else:
            score -= 2

    return score, hits


def ted_url(publication_number):
    safe = publication_number.replace("/", "-")
    return f"https://ted.europa.eu/en/notice/-/detail/{safe}" if safe else "https://ted.europa.eu/"


def render_html(payload):
    blocks = []
    for profile in payload["profiles"]:
        rows = []
        for item in profile["matches"]:
            rows.append(
                f"""
                <article class='card'>
                  <div class='score'>{item['score']}</div>
                  <div>
                    <h3><a href='{escape(item['url'])}' target='_blank' rel='noopener'>{escape(item['title'])}</a></h3>
                    <p>{escape(item['buyer'] or 'Buyer not listed')} · {escape(item['place'] or 'Location not listed')}</p>
                    <p class='meta'>Published {escape(item['publication_date'] or '—')} · Deadline {escape(item['deadline'] or '—')}</p>
                    <p class='hits'>Matched: {escape(', '.join(item['hits']) or 'general profile fit')}</p>
                  </div>
                </article>
                """
            )
        blocks.append(f"<section><h2>{escape(profile['name'])}</h2>{''.join(rows) or '<p>No strong matches today.</p>'}</section>")

    return f"""<!doctype html>
<html lang='en'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'>
<title>BidSignal Daily Digest</title>
<style>
body{{font-family:Inter,system-ui,sans-serif;background:#f7f7f5;color:#171717;max-width:920px;margin:auto;padding:32px}}
h1{{font-size:40px;margin-bottom:4px}} .sub{{color:#666;margin-bottom:32px}} section{{margin:34px 0}}
.card{{display:grid;grid-template-columns:56px 1fr;gap:16px;background:#fff;border:1px solid #e7e7e2;border-radius:16px;padding:18px;margin:12px 0}}
.score{{width:44px;height:44px;border-radius:12px;background:#171717;color:#fff;display:grid;place-items:center;font-weight:700}}
a{{color:inherit}} h3{{margin:0 0 8px}} p{{margin:5px 0;color:#555}} .meta,.hits{{font-size:14px;color:#777}}
</style></head><body>
<h1>BidSignal</h1><p class='sub'>Daily tender matches · generated {escape(payload['generated_at'])}</p>
{''.join(blocks)}</body></html>"""


def main():
    raw = post_json(TED_SEARCH_URL, QUERY)
    notices = raw.get("notices") or raw.get("results") or []
    normalized = [flatten_notice(n) for n in notices]

    output_profiles = []
    for profile in PROFILES:
        matches = []
        for notice in normalized:
            score, hits = score_notice(notice, profile)
            if score >= 2:
                matches.append({
                    **{k: v for k, v in notice.items() if k != "search_text"},
                    "score": score,
                    "hits": hits,
                    "url": ted_url(notice["publication_number"]),
                })
        matches.sort(key=lambda x: (x["score"], x["publication_date"]), reverse=True)
        output_profiles.append({"name": profile["name"], "matches": matches[:20]})

    payload = {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source": "TED",
        "profiles": output_profiles,
    }

    (DATA_DIR / "latest.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    (DATA_DIR / "latest.html").write_text(render_html(payload), encoding="utf-8")
    print(f"Fetched {len(normalized)} notices; wrote digest for {len(PROFILES)} profiles")


if __name__ == "__main__":
    main()
